<script lang="ts">
    ...
    let translation: ExperienceTranslation = $derived(
        translations[locale].experience,
    );
    ...
    function addDockAnimation() {
        ...
        const allSkillList = sectionRef.querySelectorAll(
            ".skill-list",
        ...
        for (let i = 0; i < skillItems.length; i++) {
            ...
            const onMouseMove = (event: MouseEvent) => {
                ...
                const offset =
                    Math.abs(event.clientX - itemRect.left) / itemRect.width;

                const prev =
                    (item.previousElementSibling as HTMLDivElement) || null;
                const next =
                    (item.nextElementSibling as HTMLDivElement) || null;
                ...
                if (prev) {
                    ...
                    prev.style.setProperty(
                        "--scale",
                        String(1 + scaleRange * Math.abs(offset - 1)),
                    );
                }
        ...
        for (let i = 0; i < allSkillList.length; i++) {
            ...
            cleanups.push(() =>
                allSkillList[i].removeEventListener("mouseleave", onMouseLeave),
            );
        }
...
<section
    ...
    class="flex flex-col gap-24 py-20"
>
                ...
                <div class="skill-list flex flex-wrap gap-4">
                    {#each experience.skills as skill (skill.name)}
                        ...
                        >
                            ...
                            >
                                <span
                                    class="text-xs font-semibold dark:text-neutral-50"
                                >
                                    {skill.name}
                                </span>
                            </div>
