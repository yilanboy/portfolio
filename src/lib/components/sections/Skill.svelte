<script lang="ts">
    ...
    let barBackgroundColor = $derived(
        isDarkModeEnabled ? "#404040" : "#e5e7eb",
    );
    let centerBackgroundColor = $derived(
        isDarkModeEnabled ? "#262626" : "#f9fafb",
    );
...
<section id="skill" class="flex flex-col gap-24 py-20">
    <div class="flex flex-col gap-2 text-center dark:text-neutral-50">
        <h6 class="font-caveat text-2xl md:text-4xl">
        ...
        {#each Object.entries(skills) as [key, skill] (key)}
            <div class="flex items-center justify-center">
                ...
                <Modal bind:showModal={showExperienceModal[key]}>
                    ...
                    <div
                        class="mt-4 divide-y-2 divide-dashed divide-neutral-300 dark:divide-neutral-700"
                    >
                        {#each skillExperiences[key] as experience (experience)}
                            <p
                                class="p-2 text-lg text-neutral-500 dark:text-neutral-400"
                            >
                                {experience}
                            </p>
                        {/each}
